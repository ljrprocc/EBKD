    ...
    for idx, data in enumerate(train_loader):
        ...
        for s in range(opt.G_step):
            ...
            if opt.energy == 'mcmc':
                ...
                pos_out_cls = model_G.classify(input)
                # print(pos_out_cls)
                # loss_clf.backward()
                # loss_ebm = 0.
                neg_out = model_G(neg_img, neg_id).mean()
                pos_out = model_G(input, target).mean()
                ...
                loss_clf = criterion_cls(pos_out_cls, target)
                grad_t = autograd.grad()
                # with torch.no_grad():
                feat_t_test, logit_t_test = model_t(neg_img, is_feat=True, preact=preact)
                neg_out_naive = model_G(neg_img)
                loss_cls_t = criterion_cl1s(logit_t_test, neg_id)
                up_grad = autograd.grad(loss_cls_t, neg_img) * neg_out_naive / logit_t_test * neg_id
                neg_out_naive.backward(up_grad)
                # with torch.no_grad():   
                lossa.backward()
                optimizer_ebm.step()
                # buffer.push(neg_img, neg_id_logit)
            ...
            #         # input = fake_input
            #     # input_fake = model_G(noise, target)
            #     # for st in range(opt.G_step)
            #     feat_s, logit_s = model_s(sampled_noise, is_feat=True, preact=preact)
            #     feat_t, logit_t = model_t(sampled_noise, is_feat=True, preact=preact)
            #     feat_t = [f.detach() for f in feat_t]

            # # ===================forward=====================
            # # loss_cls = criterion_cls(logit_s, target)
            # loss_div = criterion_div(logit_s, logit_t)
            # one = torch.tensor(1., dtype=torch.float).to(input.device)
            # mone = one * (-1)
            # loss_div.backward(mone)
            # optimizer_ebm.step()
            # feat_real = model_G(input, target)
            # print(neg_img.requires_grad)

        
        set_require_grad(model_G, False)
        set_require_grad(model_s, True)
        
        for s in range(opt.D_step):
