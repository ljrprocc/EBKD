...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,dilate=replace_stride_with_dilation[1], norm=norm, act=act)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2], norm=norm, act=act)
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int, stride: int = 1, dilate: bool = False, norm: str = 'none', act: str = 'relu') -> nn.Sequential:
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, norm=norm, act=act))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups, base_width=self.base_width, dilation=self.dilation, act=act, norm=norm))

        return nn.Sequential(*layers)

    def _forward_impl(self, x: Tensor, is_feat=False, preact=False, multiscale=False) -> Tensor:
